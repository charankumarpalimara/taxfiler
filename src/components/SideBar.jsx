...
export default function SideBar() {
  ...
  return (
    <div className={`fixed ${isScrolled ? 'right-0 top-1/2 -translate-y-1/2 z-40 hidden lg:flex flex-col items-center gap-0 group' : 'hidden'} `}>
      {/* Main panel */}
      <div className="bg-white flex flex-col items-center py-4 px-2.5 gap-5 rounded-l-2xl shadow-xl border border-black/5 border-r-0">

        {/* Home */}
        <a
          href="/"
          className="text-brand-purple hover:text-brand-orange transition-all hover:scale-110 flex items-center gap-1"
          title="Home"
        >
      ...
      {/* Floating CTA Text */}
      <div className="absolute right-full top-1/2 -translate-y-1/2 pr-4 opacity-0 group-hover:opacity-100 transition-all duration-300 pointer-events-none translate-x-4 group-hover:translate-x-0">
        <div className="bg-brand-purple text-white text-sm font-bold py-2 px-4 rounded-xl shadow-lg whitespace-nowrap font-sans">
          Need Help?
        </div>
      </div>
    </div>
